...
int *SumArr1 = new int[400001];
ll findNum(int i, int j, int *arr, int n);
int main() {
...
ll findNum(int i, int j, int *arr, int n) {
	...
	int mid = (i+j)/2; 
	ll leftSum = findNum(i, mid, arr,n);
	ll rightSum = findNum(mid, j, arr,n);
	ll middleSum = 0;
	//left遍历 max在左边  
	//right 计算
	int right_cur = mid+1;
	...
	//更新 
	int num = 4*n+1; 
	memset(SumArr1, 0, sizeof(int)*num);
	...
	for (int k=right_min_cur; k<=right_max_cur; k++) {
		...
		SumArr1[k+right_min_arr[k-mid]] += 1; 
	}
	while (left_cur >= i) {
	...
	for (int k=left_max_cur; k<=left_min_cur; k++) {
		...
		SumArr1[left_min_arr[k-i]-k+2*n] += 1;
	}
	...
	while (right_cur <= j) {
		...
		find_num = right_max - right_cur; 
			middleSum += SumArr1[find_num+2*n];
		right_cur++;
	}
