...
int findNum2(int i, int j, int *arr);

typedef long long ll;
int findNum(int s, int e, int *arr, vector<vector<int> > maxArr, vector<vector<int> >minArr);
int main() {
	...
	do
    {
		...
		int cnt1 = findNum(0,n-1,arr,maxArr,minArr);
		int cnt2 = findNum2(0,n-1,arr);
		if ( cnt1 != cnt2) {
			cout << cnt1 << ":" << cnt2 << endl;
			for (int i=0; i<n; i++) {
				cout << arr[i];
...
int findNum(int s, int e, int *arr,vector<vector<int> >maxArr,vector<vector<int> >minArr) {
	...
	int mid = (s+e)/2; 
	int leftSum = findNum(s, mid, arr,maxArr,minArr);
	int rightSum = findNum(mid, e, arr,maxArr,minArr);
	int middleSum = 0;
	//计算数组 

	for (int k=s; k<mid; k++) {
		for (int g=mid+1; g<=e; g++) {
			if (maxArr[k][g]-minArr[k][g] == g-k) {
				middleSum++;
			}
		}
	} 
...
int findNum2(int i, int j, int *arr) {
	...
	int mid = (i+j)/2; 
	int leftSum = findNum2(i, mid, arr);
	int rightSum = findNum2(mid, j, arr);
	int middleSum = 0;
	//left遍历 max在左边  
	//right 计算
	int right_cur = mid+1;
